...
js/
  ...
  admin-contacts.js # 管理画面: 連絡先（contacts/*.md）の一覧/編集/追加
...
contacts/
  *.md              # トップ「連絡はこちらへ」のカード（title/url 必須、handle/order/draft 任意）
gallery/
...
新規下書きの作成**がブラウザだけでできる。作品カード（つくったもの）・
ギャラリー（ドット絵の展示・画像アップロード込み）・連絡先も同じ画面から
（それぞれの章を参照）。
...
## 連絡先（連絡はこちらへ）— 追加・編集フロー

トップの「連絡はこちらへ」も同じ仕組み。`contacts/*.md`（1ファイル=1カード）を
置いて push すれば Actions がトップページを再生成する。

```markdown
---
title: GitHub                      # カードの名前（必須）
url: https://github.com/xxx        # リンク先（https:// か mailto:、必須）
handle: @xxx                       # 表示用ハンドル（任意）
order: 1                           # 表示順（小さいほど先頭。省略時は末尾）
draft: true                        # 任意。true の間は非公開
---
```

- メールは `url: mailto:you@example.com`（mailto は同一タブで開く）
- **管理画面（/admin.html）の「連絡先」欄**からも追加・編集・公開切替ができる
